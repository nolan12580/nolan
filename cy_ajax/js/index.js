$(function(){
	...
	$.ajax({
		...
		success:function(data){
			for(var i in data){
				$(".newlist").append("<div><p class='newtip left'>"+data[i].title_news+"</p><p class='right time'>"+data[i].time_news+"</p></div>")
				let ms=data[i].id_news;
				console.log(ms)
			$(".newlist>div").eq(i).click(function(){
				$(window).attr("location","newsinform.html?"+ms+"");
			})
			}
		}
	})
